...
simulator.sessionNumber = 2
simulator.msPerSecond = 20;
simulator.buffer = 0;
simulator.gameTime = new Date("2017-06-12T09:30:00.000-04:00");
simulator.pStar = 21;         //^^Time according to the game//
...
simulator.makeReadable = function(epochTime){
    let time = newDate(epochTime).toUTCString();
    return time;
}
...
}                           //^^Actual in game time as a function of real time//


var doTheThing = function() {
  initializeMarket().then(function() {
    assignOrderListener();
    assignDeleteListener();
  }).then(event=>{
    simulator.events = simulateDay(30,15,15,2,.5);
    getNextEvent();
  });
}
...
var getNextEvent = function(){
  let patsTime = new Date(simulator.currentGameTime).toUTCString();
  console.log("the current game time wrt real time is " + patsTime);
  console.log("the current game time wrt sim time is  " + simulator.gameTime);
...
let updateCurrentAssetPrice = function(lastStrikeSold) {
