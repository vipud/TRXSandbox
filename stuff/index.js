...
var paused = false;
let switchInterval;
let start1running= false;
// should be firebase key
let session = "Sessions/-Kq8seTO_LoPLOKjN8Um";
...
if (THISISLOCAL){
  ...
  */
  ...
  .ref("Sessions/{sessionId}/StartSim/start").onWrite(event => {
    if(!event.data.val()){
      return null;
    }
  ...
  */
  exports.runBeforeSim = functions.database.ref("Sessions/{sessionId}/GameParams").onWrite(event =>{
    ...
    if(IMPLEMENTED) {
      return admin.database().ref("Sessions/").child(sessionId).child("/Info").once('value').then(function(snapshot) {
        let info = snapshot.val();
        return admin.database().ref("PlayableSessions/").child(sessionId).set(info);
      }).then(function() {
        return doTheSuperThing().then(function() {
          return admin.database().ref("Sessions/").child(sessionId).child("/StartSim/")
  ...
  exports.start1 = functions.database.ref("Sessions/{sessionId}/ImportantData/function").onWrite(event => {
    ...
    if(functionNum == 1) {
      ...
      //   SessionData.secsRunning++;
      //   console.log("secsRunning in timeout: " + SessionData.secsRunning);
      // }, 1000);
      return loadGame().then(function(){
        return getNextEvent(4);
      });
    }
  });
...
let userTransaction = function(userID, type, assetID, strike, transactionVolume) {
  return postUserTransaction(userID, type, assetID, strike, transactionVolume).then(function() {
...
Returns: Promise
*/
let postUserTransaction = function(userID, type, assetID, strike, transactionVolume) {
...
let calcThisPosition = function(userID, strike, transactionVolume, type, currentPos, newPos, shares) {
  ...
  let ref = admin.database().ref(session).child("/Users/").child(userID).child("Assets/").child("INTC/")
    .child("Stats/");
  let shortTermAccumulator = 0;
  let shortTermVolume = 0;
  return ref.child("ShortTermAccumulator/").transaction(data=>{
    if(currentPos !== newPos) {
      shortTermAccumulator = 0;
      return shortTermAccumulator;
    } else {
      if(newPos == "Long" && type == "Bid") {
        shortTermAccumulator = data + parseFloat((parsedStrike*transactionVolume).toFixed(2));
        return shortTermAccumulator;
      } else if(newPos == "Short" && type == "Ask") {
        shortTermAccumulator = data + parseFloat((parsedStrike*transactionVolume).toFixed(2));
        return shortTermAccumulator;
      }
    }
    ...
    return ref.child("ShortTermVolume/").transaction(data=>{
      if(currentPos !== newPos) {
        ...
        if(newPos == "Long" && type == "Bid") {
          shortTermVolume = data + parseFloat((transactionVolume).toFixed(2));
    ...
    if(newPos == "Long" && type == "Bid") {
      return ref.child("BuyTotalVolume/").once('value').then(function(snapshot) {
        let buyTotalVolume = snapshot.val();
        return ref.child("ThisPosition/").transaction(data=>{
          if(buyTotalVolume === 0) {
            return 0;
          }
          return parseFloat((shortTermAccumulator/buyTotalVolume).toFixed(2));
        });
      });
    } else if(newPos == "Short" && type == "Ask") {
      return ref.child("SellTotalVolume/").once('value').then(function(snapshot) {
        let sellTotalVolume = snapshot.val();
        return ref.child("ThisPosition/").transaction(data=>{
          if(sellTotalVolume === 0) {
            return 0;
          }
          return parseFloat((shortTermAccumulator/sellTotalVolume).toFixed(2));
        });
      });
    }
  });
...
let calcPosition = function(userID, strike, transactionVolume, type) {
  ...
  let ref = admin.database().ref(session).child("/Users/").child(userID).child("Assets/").child("INTC/")
    .child("Stats/");
    return ref.child("Shares").once('value').then(function(snapshot) {
      shares = snapshot.val();
    }).then(function() {
      return ref.child("Position/").transaction(data=> {
        currentPos = data;
        if(shares + transactionVolume > 0) {
          newPos = "Long";
        } else if (shares === 0) {
          newPos = "None";
        } else {
      ...
      return calcThisPosition(userID, strike, transactionVolume, type, currentPos, newPos, shares);
    });
        ...
        for(let i = 0; i < userIds.length; i++) {
          proms.push(
            admin.database().ref(session).child("/Users/").child(userIds[i]).child("/Assets/Cash").once('value').then(function(snapshot){
              ...
              return admin.database().ref(session).child("/Users/").child(userIds[i]).child("/Assets/").child("INTC/").child("Stats/Shares").once('value').then(function(snapshot) {
                ...
                return admin.database().ref(session).child("/Users/").child(userIds[i]).child("/Assets/").child("INTC/").child("Stats/PL").transaction(data=>{
                  return parseFloat(cash) + (shares * SessionData.lastPrice);
                });
              });
            })
          );
...
let fulfillTransaction = function(order, newOrder, strike, orderbook){
  ...
  if(!simulator.inASimulation) {
    return Promise.resolve(userTransaction(newOrder.UserId, newOrder.Type, newOrder.AssetId, strike, transactionVolume)).then(function() { // write Receipts function
      return userTransaction(order.UserId, order.Type, order.AssetId, strike, transactionVolume);  // write Receipts function
    }).then(function() {
      return createTransactionRecord(order, newOrder, strike, transactionVolume); //global Transaction Record
    }).then(function() {
